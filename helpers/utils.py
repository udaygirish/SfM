...
class FileHandler:
    def __init__(self, data_path="../../../P3Data/", calib_path = "../../../P3Data/calibration.txt",output_path = "./Results", base_path="./Data/IntermediateOutputImages/"):
        """
    ...
    def readImgs(self):
        ...
        items_list = os.listdir(self.data_path)
        items_list.sort()

        for item in items_list[:]:
            if not (item.endswith(".png") or item.endswith(".jpg") or item.endswith(".jpeg")):
                items_list.remove(item)

        imgs_list = items_list

        print("Image List: ", imgs_list)

        imgs = []
        for img in imgs_list:
            imgs.append(cv2.imread(self.data_path + img))
        
        imgs = np.array(imgs)
        return imgs
    
    def loadCameraInt(self):
        """
        Load camera intrinsic matrix K from the specified data path.
        """
        K = []
        ...
        with open(self.calib_path, 'r') as file:
            for row in file:
                elements = row.split()
                K.append([float(elements[i]) for i in range(3)])
